...
%s
                </section>""" %(nth, "\n".join([" ".join(pos_token) for pos_token in pos])))
            nth += 1
        ...
        if chunk != []:
            ...
            annots.append("""
                <section id="content%i" class="tab-content">
%s
                </section>""" %(nth, "\n".join([" ".join(chunk_token) for chunk_token in chunk])))
            nth += 1
        ...
        if ner != []:
            ...
            annots.append("""
                <section id="content%i" class="tab-content">
%s
                </section>""" %(nth, "\n".join([" ".join(ner_token) for ner_token in ner])))
            nth += 1
...
"""
    ...
    def add_annotation_document(self, document, escaped_tokens, escaped_nontokens, column):
        ...
        new_content = "".join(escaped_tokens)
