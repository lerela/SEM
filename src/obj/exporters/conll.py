...
class Exporter(DefaultExporter):
    ...
    def document_to_unicode(self, document, couples, **kwargs):
        ...
        if not couples or (len(couples)==1 and (list(couples.keys())[0].lower() in ["word", "token"])):
            return str(document.corpus)
        else:
            lower  = {}
            fields = []
            ...
            return document.corpus.str(fields)
    
    def corpus_to_unicode(self, corpus, couples, **kwargs):
        values = list(couples.values())
        return "\n\n".join(["\n".join(["\t".join([fields for i,fields in enumerate(token) if i in values]) for token in sentence]) for sentence in corpus])
