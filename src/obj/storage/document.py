...
class Document(Holder):
    ...
    def write(self, f, depth=0, indent=4):
        f.write('%s<document name="%s">\n' %(depth*indent*" ", self.name))
        depth += 1
        f.write('%s<content>%s</content>\n' %(depth*indent*" ", self.content))
        f.write('%s<segmentations>\n' %(depth*indent*" "))
        refs = [seg.reference for seg in list(self.segmentations.values()) if seg.reference]
        for seg in sorted(list(self.segmentations.values()), key=lambda x: (x.reference and x.reference.reference in refs, x.name)): # TODO: create a sort_segmentations method to order them in terms of reference.
            depth += 1
            ...
            f.write('%s<segmentation name="%s"%s>\n' %(depth*indent*" ", seg.name, ref_str))
            depth += 1
            for element in seg:
                f.write('%s<s s="%i" l="%i" />\n' %(depth*indent*" ", element.lb, len(element)))
            depth -= 1
            f.write('%s</segmentation>\n' %(depth*indent*" "))
            depth -= 1
        f.write('%s</segmentations>\n' %(depth*indent*" "))
        
        f.write('%s<annotations>\n' %(depth*indent*" "))
        for annotation in list(self.annotations.values()):
            depth += 1
            f.write('%s<annotation name="%s" reference="%s">\n' %(depth*indent*" ", annotation.name, (annotation.reference if type(annotation.reference) in (str, str) else annotation.reference.name)))
            depth += 1
            for tag in annotation:
                f.write('%s<tag v="%s" s="%i" l="%i"/>\n' %(depth*indent*" ", tag.value, tag.start, len(tag)))
            depth -= 1
