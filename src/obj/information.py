...
tmp                = '.'.join(tmp[tmp.index("obj") : ]).rsplit(".",1)[0]
...
_train = set(["train", "eval", "evaluate", "evaluation"])
_label = set(["label", "annotate", "annotation"])
...
class Informations(object):
    def __init__(self, path=None, mode="label"):
        self._mode     = mode
