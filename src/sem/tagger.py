...
from sem.pretreatment.segmentation import segmentation, document_segmentation
from sem.pretreatment.enrich       import enrich_file, document_enrich
from sem.posttreatment.clean_info  import clean_info, document_clean
from sem.posttreatment.export      import export
from sem.posttreatment.textualise   import textualise
...
class Tagger(object):
    def __init__(self, masterfile, file_name, fast = False):
        ...
        current_output = "" # the current output in the pipeline
    ...
    def tag_file(self, file_name, directory=None):
        ...
        if self.options.format == "text":
            ...
            raise ValueError("unknown format: %s" % self.options.format)
