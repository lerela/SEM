...
def compile_dictionary(infile, outfile, kind="token",
    ...
    compile_dictionary_logger.info('compiling %s dictionary from "%s" to "%s"' %(kind, infile, outfile))
    
    try:
        dictionary_compile = _compile[kind]
    except KeyError: # invalid kind asked
        compile_dictionary_logger.exception("Invalid kind: %s. Should be in: %s" %(kind, ", ".join(list(_compile.keys()))))
        raise
