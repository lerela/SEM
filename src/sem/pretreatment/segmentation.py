...
def segmentation(infile, tokeniser_name, outfile,
    ...
    document = Document(str(infile, errors="replace"), codecs.open(infile, "rU", ienc).read().lstrip("\ufeff")) # removing BOM
    ...
    segmentation_logger.info('writing to "%s"', outfile)
    
    joiner = (" " if output_format=="line" else "\n")

    for sentence in sentences:
        outfile.write(joiner.join(sentence) + "\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-encoding", dest="ienc",
                        help="Encoding of the input (default: UTF-8)", default="utf-8")
    ...
    with codecs.open(parser.infile, "r", ienc) as infile, codecs.open(outfile, "w", parser.oenc) as outfile:
        segmentation(infile.read().lstrip("\ufeff"), args.tokeniser_name, outfile,
